...
This is a native Android app scaffolded from Android Studio's default "Empty Activity" (Jetpack Compose) template. The application ID/namespace is `com.joshuawallis.mp3player`, but no MP3-player-specific functionality has been implemented yet — `MainActivity.kt` still contains the template's `Greeting("Android")` composable. Treat the existing code as a starting skeleton, not an established architecture to preserve.
...
- Run a single unit test class: `./gradlew testDebugUnitTest --tests "com.joshuawallis.mp3player.ExampleUnitTest"`
...
- Compose theming lives under `app/src/main/java/com/joshuawallis/mp3player/ui/theme/` (`Color.kt`, `Theme.kt`, `Type.kt`).
